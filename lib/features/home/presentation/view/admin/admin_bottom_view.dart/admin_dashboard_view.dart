...
class _AdminDashboardViewState extends ConsumerState<AdminDashboardView> {
  ...
  Future _secondBrowseImage(WidgetRef ref, ImageSource imageSource) async {
    try {
      final image = await ImagePicker().pickImage(source: imageSource);
      if (image != null) {
        setState(() {
          _SecondImg = File(image.path);
        });
      } else {
        return;
      }
  ...
  void _secondShowBottomCameraSheet() {
    showModalBottomSheet(
      backgroundColor: Colors.grey[300],
      context: context,
      isScrollControlled: true,
        ...
        child: Row(
          ...
          children: [
            ElevatedButton.icon(
              onPressed: () {
                checkCameraPermission();
                _secondBrowseImage(ref, ImageSource.camera);
                Navigator.pop(context);
                // Upload image it is not null
              },
              icon: const Icon(Icons.camera),
            ...
            ElevatedButton.icon(
              onPressed: () {
                _secondBrowseImage(ref, ImageSource.gallery);
                Navigator.pop(context);
              },
              icon: const Icon(Icons.image),
              label: const Text('Gallery'),
              ...
              Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    InkWell(
                      onTap: _secondShowBottomCameraSheet,
                      child: SizedBox(
                        height: 200,
                        width: 200,
                        child: CircleAvatar(
                          radius: 50,
                          backgroundImage: _SecondImg != null
                              ? FileImage(_SecondImg!)
                              : const AssetImage('assets/images/logo.png')
                                  as ImageProvider,
                        ),
                      ),
